    ...
    async (categoryId: number, categoryName: string, pageNumber: number = 1) => {
      ...
      try {
        if (categoryName === 'All') {
          ...
          console.log(`\x1b[32m[SUCCESS] Fetched ${allNews.length} items for ${categoryName}\x1b[0m`);
          setNewsData(prev => {
        ...
        if (error instanceof Error) {
          ...
          console.log(`\x1b[31m[ERROR] Unknown error fetching news for ${categoryName}\x1b[0m`);
        }
      } finally {
        setLoading(prev => {
